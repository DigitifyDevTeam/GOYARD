...
function ParisCompactDevisForm({
  ...
  const showVolumeAutomaticCalc = config.fieldSlug === "paris";
  ...
  const volumeKnown = form.volume.trim() !== "";
    ...
    try {
      ...
      const payload = {
        ...
        options_depart: {
          ...
          liste_objets: form.liste_objets,
  ...
  return (
    ...
    >
      ...
      <div className={cn("space-y-3", isMobile ? "mt-5" : "mt-6")}>
        ...
        {showVolumeAutomaticCalc ? (
          <div className="w-full">
            <button
              ...
              className={cn(
                "w-full rounded-xl border border-[#CC922F] bg-white px-4 py-2.5 text-center font-['Poppins',sans-serif] font-extrabold text-[#CC922F] transition",
                "hover:bg-[#CC922F]/10 focus:outline-none focus:ring-2 focus:ring-[#CC922F]/40 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-white",
                isMobile ? "text-[15px] leading-snug" : "text-base",
              )}
              aria-label="Calculer mon volume automatiquement"
            >
              Calculer mon volume automatiquement
            </button>
            {volumeKnown ? (
              <p className="mt-1.5 text-center font-['Poppins',sans-serif] text-xs text-slate-400">
                Volume déjà renseigné — videz le champ pour utiliser le calcul automatique.
              </p>
            ) : null}
          </div>
        ) : null}
        ...
        {showVolumeAutomaticCalc ? (
          <IconField icon={ClipboardList}>
            <textarea
              id={fieldId("liste-objets")}
              value={form.liste_objets}
              onChange={set("liste_objets")}
              placeholder="Liste Objets à déménager"
              rows={3}
              aria-label="Liste Objets à déménager"
