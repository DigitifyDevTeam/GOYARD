...
Replace `localhost:8000` with your server IP or domain if testing from another machine.
...
```bash
curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:8000/api/demenagement/clients/
```
...
```bash
curl -s http://127.0.0.1:8000/api/demenagement/clients/
```
...
```bash
curl -s -w "\nHTTP_CODE:%{http_code}\n" -X POST http://127.0.0.1:8000/api/demenagement/client-info/ \
...
## 4. If the frontend calls the API on another host (e.g. domain vs :8000)

From your **local machine** (replace `YOUR_SERVER_IP` or use the domain):

```bash
curl -s -X POST http://YOUR_SERVER_IP:8000/api/demenagement/client-info/ \
  -H "Content-Type: application/json" \
...
- **Wrong URL in the app**: frontend must call the same host/port as the backend (e.g. both on `goyard-demenagement.fr` with `/api/` or both on `IP:8000`).
